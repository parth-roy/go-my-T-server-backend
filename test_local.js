const env = Object.assign({}, process.env, { NODE_ENV: 'development', PORT: 8889, RAZORPAYX_PAYOUTS_ENABLED: 'false', MULTI_PARTY_TRANSFERS_ENABLED: 'false', PRIVATE_BID: 'false' });
const { spawn } = require('child_process');
const server = spawn('node', ['dist/server.js'], { env, cwd: process.cwd(), shell: true });
...
setTimeout(async () => {
  try {
    const API = "http://localhost:8889/api/v1";
