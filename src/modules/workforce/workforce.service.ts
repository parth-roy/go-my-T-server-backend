  ...
  let openBookings = gigs
    ...
    .map(g => {
      ...
      return {
        ...
        laborType:      g.gigCategory || g.gigType,
        gigCategory:    g.gigCategory,
        locationZone:   g.locationZone,
        durationHours:  g.durationHours,
        urgency:        g.urgency,
        payoutAmount:   g.perWorkerRate > 0 ? g.perWorkerRate : (g.totalFare ?? 0) / totalSlots,
        slotsRemaining: totalSlots - acceptedCount,
        totalSlots,
        vehicleType:    'Gig',
        goodsType:      g.description && !g.description.startsWith('{') ? (g.description.includes('—') ? g.description.split('—')[0].trim() : g.description) : (g.gigCategory || 'Service'),
  ...
  if (gigAssignment) {
    ...
    return {
      ...
      booking: {
        ...
        laborType: gigAssignment.gig.gigCategory || gigAssignment.gig.gigType,
        gigCategory: gigAssignment.gig.gigCategory,
        laborersCount: gigAssignment.gig.workersNeeded,
        vehicleType: 'Gig',
        goodsType: gigAssignment.gig.description && !gigAssignment.gig.description.startsWith('{') ? (gigAssignment.gig.description.includes('—') ? gigAssignment.gig.description.split('—')[0].trim() : gigAssignment.gig.description) : (gigAssignment.gig.gigCategory || 'Service'),
...
export async function getNearbyPins(userId: string, query: JobRadarQuery) {
  ...
  const availablePins = jobs
    ...
    .map(job => ({
      ...
      perWorkerRate: job.perWorkerRate,
      laborCharge: job.perWorkerRate > 0 ? job.perWorkerRate : (job.totalFare ?? 0) / (job.workersNeeded ?? 1),
      laborType: job.gigCategory || job.gigType,
      gigCategory: job.gigCategory,
