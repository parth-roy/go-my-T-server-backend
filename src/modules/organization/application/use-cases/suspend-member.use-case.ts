...
import { OrganizationMemberSuspendedEvent } from '../../domain/events/organization-member-suspended.event';
...
export class SuspendMemberUseCase {
  ...
  async execute(context: RequestContext, targetMemberId: string) {
    ...
    // Authorize
    MembershipPolicy.assertCapability(actorCapabilities, 'SUSPEND_MEMBER');
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // Rule: cannot suspend self
      MembershipPolicy.assertNotSelf(member.getUserId(), context.user.id, 'suspend');

      const previousState = member.getStatus();

      // The aggregate ensures we don't suspend a terminated or primary owner
      ...
      const event = new OrganizationMemberSuspendedEvent(
      ...
      console.log('[EVENT] OrganizationMemberSuspendedEvent dispatched for', targetMemberId);
    });
  }
}
